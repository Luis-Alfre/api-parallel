...
/*
Permite extraer coordenadas de un archivo de texto formato TSP
*/	
int extraerCooordenadas(char cadena[], char indice[], char cX[], char cY[])
{
...
int leerArchivo(char nombreArchivo[], int intIndice[], int cIntX[], int cIntY[])
{
...
//	while (fscanf(in_file, "%[^\n] ", file_contents) != EOF)
    while (!feof(in_file)) {
        ...
        //printf("> %s\n", file_contents);
       	if (cont>=6)
        	{
       			strcpy(cadena, file_contents);
        		extraerCooordenadas(cadena, indice, cX, cY);
				int indiceInt = (int) strtol(indice, NULL, 10);
				int cXInt = (int) strtol(cX, NULL, 10);
				int cYInt = (int) strtol(cY, NULL, 10);
				intIndice[tam] = indiceInt;
				cIntX[tam] = cXInt;
				cIntY[tam] = cYInt;
				tam++;
			}
	...
	char charX[]= "coordenadasX.txt";
	imprimirVectorEnteroFichero(cIntX,tam,charX);
	char charY[] = "coordenadasY.txt";
	imprimirVectorEnteroFichero(cIntY,tam,charY);
   fclose(in_file);
...
*/
void leerValorEnteroArchivo(char nombreArchivo[], 
					int coordenadasX[], int nCiudades){
...
void copiarVectorReales(double M[], double N[], int n)
{
	for (int i=0;i<n;i++)
	{
			M[i] = N[i];
...
void leerFicheroCordenadas(char filename[], int coordenadas[])
{
